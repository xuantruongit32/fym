...
//            return t;
//        }
        public void editAccount(String name, String newName, double newBalance){
            accounts.get(name).setName(newName);
            if(accounts.get(name).getBalance()< newBalance){
                addCategory("noName","Income");
                addTransaction("Income",accounts.get(name),"noName",newBalance - accounts.get(name).getBalance(),"edit",true,LocalDate.now());
...
//        }
        ...
        public float totalExpenseDaily(LocalDate date, Account account, String category){  
         ...
         public float totalExpenseMonthly(LocalDate date, Account account, String category){  
            List<Transaction> expenseList = transactions.get("Expense");
            float totalExpense = 0;
            int monthNow = date.getMonth().getValue();
            for (Transaction transaction : expenseList) {
